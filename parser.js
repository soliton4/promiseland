    ...
    */
        ...
        */
        ...
        if (this.codeAr){
          for (i = this.codeAr.length; i; --i){
            resStr += this.codeAr[i - 1].getPostCodeStr();
          };
        };
        if (this.preCode){
          resStr += this.preCode.getPostCodeStr();
        };
        return resStr;
      }
      , getPostCodeStr: function(){
        if (this.isComplete){
          return "";

        }else{
          return this._getPostCodeStr();
        };
      }
    ...
    var CodeParser = function(par){
      ...
      this.runtimeError = function(par, par2){
        ...
        return runtimeErrorNoWarning(par2);
      };
      ...
      this.findClasses = function(par){
        ...
        if (par.type == "Class"){
          ...
          if (ci.isTyped){
            ...
            if (par.body.literal){
              var literal = this.createClassLiteral(par.body.literal, true/* only types */);
              if (ci.hasName){
        ...
        if (dynamic){
          ...
          for (p in par){
            if (!par[p] || ignoreProps[p]){
              ...
              addProperty(p);
              res.push(this.makeCompleteStatementDynamic(par[p]));
            };
          };
          res.push("})");
          
          var dynRes = this.newResult();
          dynRes.push(this.makeCompleteStatement(res), undefined, {
            dynamic: true
          });
          return dynRes;
